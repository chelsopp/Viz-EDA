...
![](visualization_1_files/figure-gfm/unnamed-chunk-4-1.png)<!-- --> the
first code is just saving my scatterplot to the environment under a
variable, the second part of the code calls the variable and I can see
it on my console.
...
`color = name` basically automatically picks a color for each `name`
variable `geom_smooth` adds statistical info ontop of plot
...
where you define aethetics can matter - i also define aesthestics down
in `geom_point` using size in `aes`, you can use another variable to
determine the size
...
use faceting real quick aka separating scatterplot into there separate
plots by name `~ .`means row ~ column, e.g. `name ~ .` means put rows in
mean, then leave column blank
...
![](visualization_1_files/figure-gfm/unnamed-chunk-17-2.png)<!-- -->
...
![](visualization_1_files/figure-gfm/unnamed-chunk-23-2.png)<!-- -->
